...
boolean catalog=false,bookName=false,author=false,subject=false,placeOnShelf=false,editionNumber=false,allFlagStatus=false;
...
private static Book book;
    ...
    void UpdateBookAction(ActionEvent event) {
    	if(catalog&&!allFlagStatus) {
			...
			if(IGUIcontroller.CheckOnlyLetter(AuthorTextField,AuthorLabel,OnlyThisLetters,OnlyThisLetterError) &&IGUIcontroller.CheckIfUserPutInput(AuthorTextField,AuthorLabel) ) {
				counter++;
	    		if(author) {
			...
			if(IGUIcontroller.CheckOnlyLetter(SubjectTextField,SubjectLabel,OnlyThisLetters,OnlyThisLetterError) &&IGUIcontroller.CheckIfUserPutInput(SubjectTextField,SubjectLabel) ) {
			counter++;	
    		if(subject) {
